    ...
    if (grid) {
      grid.innerHTML = data.team.map((m, i) => `
        <div class="lift-card flex flex-col items-center gap-sm bg-surface-container-lowest border border-outline-variant rounded-lg p-md text-center" data-aos="fade-up" data-aos-delay="${i * 100}">
          ...
          ${m.email ? `<span class="font-body-sm text-body-sm text-on-surface-variant">${m.email}</span>` : ''}
          ${m.linkedin ? `<a href="${m.linkedin}" target="_blank" rel="noopener" title="LinkedIn" class="mt-1 inline-flex items-center justify-center w-7 h-7 rounded-md bg-[#0A66C2] hover:opacity-80 transition-opacity">
